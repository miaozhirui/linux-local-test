/*! hframework dom API - v2.1.0 based on intel-appframework - v2.1.0 - 2014-08-27 */
...
if (!window.af || typeof (af) !== "function") {
    ...
    var af = (function(window) {
        ...
        var $afm = function(toSelect, what) {
            ...
            return this.selector(toSelect, what); // toSelect is string
        };
        ...
        $.isArray = function(obj) {
            ...
            //ios 3.1.3 doesn"t have Array.isArray
        };
        ...
        $.fn = $afm.prototype = {
            ...
            html : function(html, cleanup) {
                ...
                for (var i = 0; i < this.length; i++) {
                    if (cleanup !== false)
                        $.cleanUpContent(this[i], false, true); //>>> call "destory"(customized event) handler binded to node and release other event handler
                    if (isWin8) {
                        var item = this[i];
            ...
            removeAttr : function(attr) {
                ...
                var removeFixer = function(param) {
                    that[i].removeAttribute(param); //>>> that, i defined below
            ...
            insertAfter : function(target) {
                return this.insertBefore(target, true);
            },
            ...
            offset : function() {
                ...
                return {
                    ...
                    right : obj.right + window.pageXOffset, //= obj.right = obj.left + obj.width
            ...
            parent : function(selector, recursive) {
                ...
                return this.setupOld($($.uniqueArray(elems)).filter(selector));
            },
            ...
            /**
             * return prev dom element
             ```
                 $("#foo").prev("div"); //Selector
             ```
             * @param {String} tagName
             * @param {bool} next
             * @return {Object} Returns an appframework object with the element based off the selector
             * @title $('.foo').prev(selector,[next]);
             */
            prev: function(tagName, next) {
                next = next || false;
                if(tagName) tagName = tagName.toLowerCase();
                
                var node = next ? this[0].nextSibling : this[0].previousSibling;
                for (; node; node = next ? node.nextSibling : node.previousSibling) {
                    if ((node.tagName && !tagName) || (tagName && node.tagName && node.tagName.toLowerCase() == tagName)) return $(node);
                }
            ...
            /**
             * return next dom element
             ```
                 $("#foo").next("div"); //Selector
             ```
             * @param {String} tagName
             * @param {bool} next
             * @return {Object} Returns an appframework object with the element based off the selector
             * @title $('.foo').next(selector,[next]);
             */
            next: function (tagName) { return this.prev(tagName, true); },
            ...
            filter : function(selector) {
                ...
                return this.setupOld($($.uniqueArray(elems)));
            },
            ...
            serialize : function() {
                ...
                var params = [];
                for (var i = 0; i < this.length; i++) {
                    this.slice.call(this[i].elements).forEach(serializeFn); //>>> html5:: dom element.elements
                }
        ...
        /**
         * Execute a jsonP call, allowing cross domain scripting (implemented by JS Injection)
         *
            ...
            if (options.url.indexOf("callback=?") !== -1) {
                ...
                if (options.url.indexOf("?") === -1) {
                    ...
                    if (options.url.indexOf("callback=") !== -1) {
                        ...
                        var amp = options.url.indexOf('&', offset); //>>> org code: options.url.indexOf(offset); 
                        if (amp === -1)
                            amp = options.url.length;
                        var oldCB = options.url.substring(offset, amp); //>>> org code: options.url.substr(offset, amp); 
        ...
        $.ajax = function(opts) {
            ...
            try {
                ...
                //>>> wusheng: auto cookie management (send)
                if(protocol != "file:"){
                    var vvCookie = window.localStorage.getItem('vvcookie');
                    if (vvCookie) settings.headers['vvcookie'] = escape(vvCookie);
                }
                ...
                xhr.onreadystatechange = function() {
                    ...
                    if (xhr.readyState === 4) {
                        ...
                        if ((xhr.status >= 200 && xhr.status < 300) || xhr.status === 0 && protocol === "file:") {
                            //>>> wusheng: auto cookie management (receive)
                            if(protocol != "file:"){
                                var setck = xhr.getResponseHeader('Set-vvcookie');
                                if (setck) {
                                    setck = setck.trim();
                                    window.localStorage.setItem('vvcookie', setck);
                                    $.vv.ck.ckCache = JSON.parse(setck);
                                    //flush ckCache
                                }
                            }
                        ...
                        if(xhr.uuid)delete ajaxQueue[xhr.uuid];
                    }
                ...
                if (settings.timeout > 0)
                    xhr.abortTimeout = setTimeout(function() {
                ...
                //add to queue to mannage when switch afui panel
                if(!settings.noQueue) {
                    xhr.uuid = $.uuid();
                    xhr.context = context;
                    xhr.errorCb = settings.error;
                    ajaxQueue[xhr.uuid] = xhr;
                }
        ...
        /**
         * Abort all the ajax request.
         ```
         $.abortQueuedAjax();
         ```
         */
        $.abortQueuedAjax = function(why) {
            why = why ? why : 'panelhided';
            for( var uuid in ajaxQueue) {
                var xhr = ajaxQueue[uuid];
                if(!xhr.abort) continue;
                xhr.onreadystatechange = null;
                clearTimeout(xhr.abortTimeout);
                xhr.abort();
                xhr.errorCb.call(xhr.context, xhr, why);
                delete ajaxQueue[uuid];
            };
        };
        ...
        $.param = function(obj, prefix) {
            ...
            if (obj instanceof $afm) {
                obj.each(function() {
                    var k = prefix ? prefix + "[" + this.id + "]" : this.id, v = this.value;
        ...
        function detectUA($, userAgent) {
            ...
            $.os.ios = $.os.ipad || $.os.iphone; //>>> in face default > ios7
            $.os.ios8 = $.os.ios && userAgent.match(/(8|9|10|11|12|13)_/) ? true : false;
            ...
            $.os.xwalk = 'no'; //if it is crosswalk webview
            if($.os.android) {
                $.os.androidVersion = userAgent.replace(/.*Android\s*(\d+(\.\d+)?).*/, '$1');
                $.os.xwalk = userAgent.match(/Crosswalk/) ? 'yes' : 'no';
            }
        ...
        function matcherFor(ns) {
            return new RegExp("(?:^| )" + ns.replace(" ", " .* ?") + "(?: |$)"); //>>> ?: no capturing
        }
        ...
        function add(element, events, fn, selector, getDelegate) {
            ...
            eachEvent(events, fn, function(event, fn) {
                ...
                var proxyfn = function(event) { //>>> $.fn.trigger(event, data, probs)
                    if (event.ns) {
                        var matcher = matcherFor(event.ns);
        ...
        function addDelegate(element, event, callback, selector, data) {
            add(element, event, callback, selector, function(fn) {
                return function(e) {
                    var evt, match = $(e.target).closest(selector, element).get(0); //>>> bind to the delegated element
                    if (match) {
         ...
         */
        $.fn.on = function(event, selector, data, callback) { //>>> TODO: has bug, can't capture trigger event customize data when in 'Delegate' mode
            if ($.isFunction(data)) {
                callback = data;
                data = null;
            }
        ...
        /**
         * Trigger an event to an object instead of a DOM Node//>>> it's a synchronized function
         ```
        ...
        $.proxy = function(f, c, args) {
            return function() {
                ...
                //use scope function call arguments
            };
        };
        ...
        function cleanUpNode(node, kill) {
            ...
            if (kill && node.dispatchEvent) {
                var e = $.Event("destroy", { //>>> bind a handler to "destory" event to release something should be released
                    bubbles : false
                });
        ...
        // Like setTimeout(fn, 0); but much faster(As Soon As Possible)
        var asapQueue = [];
        /**
         * This adds a command to execute in the JS stack, but is faster than setTimeout
         ```
        ...
        $.asap = function(fn, context, args) {
            ...
            asapQueue.push({fn:fn, context: context ? context : {}, params: args ? args : []});
            //post a message to ourselves so we know we have to execute a function from the queue
        ...
        window.addEventListener("message", function(event) {
            if (event.source === window && event.data === "afm-asap") {
                ...
                if (asapQueue.length > 0) {
                    var itm = asapQueue.shift();
                    itm.fn.apply(itm.context, itm.params);
                }
            }
        }, true);
        ...
        /**
         * this function load JS on demond: added by wusheng
         */
        $.loadJS = function(fileSrc, onloadCb, type, async) {
            if (remoteJSPages[fileSrc]) {
            ...
            var doc = document.createElement("script");
            doc.src = fileSrc;
            doc.type = type || "text/javascript";
            doc.async = async ? async : false;
            if (onloadCb) 
                doc.onload = onloadCb;
            document.getElementsByTagName('head')[0].appendChild(doc);
            remoteJSPages[fileSrc] = 1;
            doc = null;
        };
...
(function(root, factory) {
        ...
        var getThenableIfExists = function(obj) {
            ...
            if (obj !== null && typeof obj === "object" && typeof then === "function") {

                return function() {
                    return then.apply(obj, arguments);
                };
            }
        };
        ...
        ayepromise.defer = function() {
            var state = PENDING, outcome, thenHandlers = [];
            ...
            var safelyResolveThenable = function(thenable) {
                ...
                try {
                    thenable(onceWrapper(transparentlyResolveThenablesAndFulfill), onceWrapper(doReject));
                } catch (e) {
