/*! hframework ui based on intel-appframework - v2.1.0 - 2014-09-16 */
...
(function($) {
    "use strict";
    var sheetInstance = null; //>>>singleTon
    $.fn.actionsheet = function(opts) {
        if(sheetInstance) {
            return sheetInstance
        }
        
        if(!opts) return sheetInstance;
        
        sheetInstance = new actionsheet(this[0], opts);
        return sheetInstance;
    };
    ...
    var actionsheet = (function() {
        var actionsheet = function(elID, opts) {
            ...
            if (typeof opts === "string") {
                markup = $(markStart + opts + "<a class='cancel' data-pressed='true'>取消</a>" + markEnd);
                ...
                for (var i = 0; i < opts.length; i++) {
                    var item = $("<a data-pressed='true'>" + (opts[i].text || "TEXT NOT ENTERED") + "</a>");
            ...
            $("#af_action_mask").bind("touchmove",function(e){
                e.preventDefault();
                e.stopPropagation();
            }).bind( "touchstart",function(e){
                e.stopPropagation(); //>>>prevent Android < 4.4 clicking through
            }).bind('click', function(){
                that.hide();
                return false;
            });
        ...
        actionsheet.prototype = {
            ...
            hide: function() {
                ...
                $("#af_action_mask").off('click');
                $(this.el).find("#af_action_mask").unbind("touchstart touchmove").remove();
...
(function($) {
    ...
    var getCSS3Animate = function(obj, options) {
        ...
        id = objId(el);
        if (cache[id]) {
            if(options)cache[id].animate(options);
            tmp = cache[id];
        } else {
            if(options) {
    ...
    $.fn.css3Animate = function(opts) {
        ...
        if (opts && !opts.complete && opts.callback) opts.complete = opts.callback;
        //first on
        ...
        css3Animate.prototype = {
            animate: function(options) {
                ...
                if (classMode) {
                    ...
                    if (!options.origin) options.origin = "0% 0%"; //>>> transform-origin for element to be roatated
...
(function($) {
    ...
    Animate.prototype = {
        ...
        animate: function(options) {
            var that=this;
            if(that.isAnimating && !options.clearDoing) return;
            ...
            this.expTime=this.startTime + this.runTime;
            this.startMatrix=$.getCssMatrix(this.el);
            this.distX = options.x - this.startMatrix.e;
            this.distY = options.y - this.startMatrix.f;
        ...
        doAnimate:function(){
            ...
            if (this.runTime===0 || (now >= this.expTime)) {
            ...
            nextX = this.distX * easeStep + this.startMatrix.e;
            nextY = this.distY * easeStep + this.startMatrix.f;
...
/**
 * af.passwordBox - fixes known issues with input type="password" on Android <4 when -webkit-transform:translate3d is applied to an ancestor.
...
(function ($) {
    "use strict";
    var HIDE_REFRESH_TIME = 350; // hide animation of pull2ref duration in ms
    var cache = [];
    var objId = function (obj) {
    ...
    function bindTouchLayer() {
        ...
        if ($.os.android && !$.os.chrome && $.os.webkit) {
            var androidFixOn = false; //>>>not used
    ...
    var scroller = (function () {
        ...
        var scroller = function (elID, opts) {
            ...
            //>>> added by wusheng
            opts.scrollBars = opts.scrollBars ? true : (el.getAttribute('data-scrbar') == 'y' ? true : false);
        ...
        scrollerCore.prototype = {
            ...
            refresh: false,
            pullRefreshTip:'<i class="fa fa-arrow-down" ' +($.os.android ? 'style="display:none;"' : '')+ '></i><e>下拉刷新...</e>',
            releaseRefreshTip:'释放刷新...',
            refreshHeight: 60,
            refreshElement: null,
            ...
            moved: false,
            eventsActive: false, //scroller enable or disable switcher
            rememberEventsActive: false, //used by lock() to rember the preverious state
            ...
            _scrollTo: function (params, time, recur) {
                var now = Date.now(), easeStep, that = this;
                if(recur !== true) {
                    if(time < 16.7 || isNaN(time) || (this.el.scrollTop == params.y && this.el.scrollLeft == params.x)) {
                        this.el.scrollTop = params.y;
                        this.el.scrollLeft = params.x;
                        this.scrollToDoing = false;
                        window.cancelAnimationFrame(this.scrollToTimer);
                        if(Math.abs(this.el.scrollHeight) > 20000) { //iOS bug(the list will hidden when list too long when scroll finished)
                            this.el.scrollTop += 1;
                            window.setTimeout(function(){that.el.scrollTop -= 1;}, 1);
                        }
                        return;
                    }
                    
                    window.cancelAnimationFrame(this.scrollToTimer);
                    this.scrollToParams.startTime   = now;
                    this.scrollToParams.expTime     = now + time;
                    this.scrollToParams.startScrollTop    = this.el.scrollTop;
                    this.scrollToParams.startScrollLeft   = this.el.scrollLeft;
                    this.scrollToParams.distY = params.y - this.el.scrollTop;
                    this.scrollToParams.distX = params.x - this.el.scrollLeft;
                    this.scrollToDoing = true;
                } else if(now >= this.scrollToParams.expTime) {
                    this.el.scrollTop = params.y;
                    this.el.scrollLeft = params.x;
                    this.scrollToDoing = false;
                    window.cancelAnimationFrame(this.scrollToTimer);
                    if(Math.abs(this.el.scrollHeight) > 20000) {
                        this.el.scrollTop += 1;
                        window.setTimeout(function(){that.el.scrollTop -= 1;}, 1);
                    }
                    return;
                ...
                easeStep = (now - this.scrollToParams.startTime) / time;
                easeStep = easeStep > 1 ? 1 : easeStep;
                
                if(this.scrollToParams.startScrollTop != params.y) 
                    this.el.scrollTop = this.scrollToParams.startScrollTop + this.scrollToParams.distY * easeStep ;
                if(this.scrollToParams.startScrollLeft != params.x) 
                    this.el.scrollLeft = this.scrollToParams.startScrollLeft + this.scrollToParams.distX * easeStep ;
                
                if(this.scrollToDoing)
                    this.scrollToTimer = window.requestAnimationFrame(function(){that._scrollTo(params, time, true);});
            },
            ...
            handleEvent: function (e) {
                if (!this.scrollingLocked) {
                    ...
                    case "touchstart":
                        ...
                        this.moved = false;
                        if(e.target.getAttribute("no-scroll")) return e.preventDefault(); //>>> use no-scroll to prevent sth scrolling when ... 
                        this.onTouchStart(e);
                        if(!this.bubbles)
                            e.stopPropagation();
                        break;
            ...
            coreAddPullToRefresh: function (rEl) {
                ...
                if (this.refreshElement === null) {
                    ...
                    if (orginalEl !== null) {
                        ...
                        afEl = $("<div id='" + this.container.id + "_pulldown' class='afscroll_refresh'>" + this.pullRefreshTip + "</div>");
                    }
                } else {
                ...
                //>>> $.os.ios may cause some problem (android4.4 can't add backface bcs can cause issue performance)
                this.refreshContainer = $("<div style='overflow:hidden;height:0;display:none;"+($.os.ios ? "width:100%;-webkit-backface-visibility:hidden !important;-webkit-transition:none;transition:none;" : "") + "'></div>"); 
                
                $(this.el).prepend(this.refreshContainer.prepend(el));
                this.refreshContainer = this.refreshContainer[0];
            },
            
            fireRefreshRelease: function () {
                if (!this.refresh) return;
                this.setRefreshContent('<img src="./img/loading.gif">', 'to_refresh');
                $.trigger(this, "refresh-release", [true]) !== false; //>>> it's a synchronized function
                this.preventHideRefresh = false;
                this.refreshRunning = true;
            },
            
            setRefreshContent: function (content, toAct) {
                if(!$.ui.pullRefreshEffect) $(this.container).find(".afscroll_refresh").html(content);
                else {
                    if(toAct == 'to_pull') {
                        $(this.container).find(".afscroll_refresh").removeClass('to_release to_pull').html(content);
                    } else if(toAct == 'to_release') {
                        $(this.container).find(".afscroll_refresh").addClass('to_release').find('e').html(content);
                    } else if(toAct == 'to_refresh') {
            ...
            scrollToItem: function (el, where) { //TODO: add functionality for x position
                ...
                if (where === "bottom") { //>>> align item bottom with the bottom of the scroll container
                    itemPos = el.offset();
                    newTop = itemPos.bottom - this.afEl.offset().bottom;
            ...
            correctScroll: function(time){ //>>> added by wusheng
                time = time || 0;
                if(this.el.scrollHeight < (Math.abs(this.scrollTop) + this.container.clientHeight)) this.scrollToBottom(time);  
            }
        };
        ...
        nativeScroller.prototype.enable = function (firstExecution) {
            ...
            this.el.addEventListener("touchstart", this, false); //>>> notice the 2nd param is Obj
        ...
        nativeScroller.prototype.addPullToRefresh = function (el, leaveRefresh) {
            ...
            if (this.refresh && this.refresh === true) {
                ...
                this.refreshContainer.style.top = "0"; //-60px way absoleted, bcs on iOS8 it doesn't work
                this.refreshContainer.style[$.feat.cssPrefix + "Transform"] = "translate3d(0, -60px, 0)";
        ...
        nativeScroller.prototype.onTouchStart = function (e) {
            ...
            if(this.verticalScroll){
                if(this.el.scrollTop === 0) {
                    this.el.scrollTop = 1; //or it will not scroll passed by the top(iOS bug)
                    this.yReset = -1;
                }
                if(this.el.scrollTop === (this.el.scrollHeight - this.el.clientHeight)) { // scrolled to bottom
                    this.el.scrollTop -= 1; //or it will not scroll passed by the bottom(iOS bug)
                    this.yReset = 1;
                }
            }
            ...
            this.dY = e.touches[0].pageY; //document Y offset
        ...
        nativeScroller.prototype.onTouchMove = function (e) {
            ...
            //console.log('this.dY==> '+this.dY+' this.startTop==> '+this.startTop+' newCY==> '+newcY);

            //var scorllTop
            var atBottom = (this.el.scrollHeight - this.el.scrollTop) === this.el.clientHeight && newcY < 0;
            ...
            if(this.horizontalScroll && this.startLeft === 0 && this.el.scrollLeft === 0 && newcX > 0){
                preventDefault && e.preventDefault();
            }

            if(this.verticalScroll && atBottom){
                preventDefault && e.preventDefault();
            }
            ...
            } else if (this.refresh && this.refreshTriggered && (
                (this.refreshRunning && (this.el.scrollTop > this.refreshHeight)) || 
                (!this.refreshRunning && (this.el.scrollTop > -this.refreshHeight)) )) {
                    
                this.refreshTriggered = false;
                //this.refreshRunning = false;
                this.hideRefresh(false);
                this.setRefreshContent(this.pullRefreshTip, 'to_pull');
            ...
            //console.log('atBottom==> '+atBottom+ ' this.initScrollProgress==> '+this.initScrollProgress+' el.scrollTop==> '+this.el.scrollTop);
        };
        
        nativeScroller.prototype.showRefresh = function () {
            if (!this.refreshTriggered) {
                this.refreshTriggered = true;
                this.setRefreshContent(this.releaseRefreshTip, 'to_release');
        ...
        nativeScroller.prototype.onTouchEnd = function () {
            ...
            var that = this;
            triggered && this.fireRefreshRelease();
            if(!this.moved){
            ...
            if (triggered && this.refresh) {
                ...
                //iOS has a bug that it will hide the refresher when refresh-release, so add a fake element while we reset the pull to refresh position
                ...
                $(this.el).append(tmp);
                // that.refreshContainer.style.top = "0px"; //>>> delted by wusheng
                that.refreshContainer.style[$.feat.cssPrefix + "Transform"] = "translate3d(0, 0, 0)";
                that.refreshContainer.style.position = "";
            ...
            if (this.infinite && !this.infiniteTriggered && ((this.el.scrollTop) >= (this.el.scrollHeight - this.el.clientHeight - 40))) {
            ...
            this.logPos(this.el.scrollLeft, this.el.scrollTop);
            var counter = 0, 
                counterThresh = parseInt(max/8, 10);
            ...
            self.nativePolling = setInterval(function () {
                ...
                //console.log('=====================> polyfill:: currEleScrollTop ==> '+self.el.scrollTop);
                if (self.el.scrollTop !== currPos.top || self.el.scrollLeft !== currPos.left) {
                    clearInterval(self.nativePolling);
                    self.logPos(self.el.scrollLeft, self.el.scrollTop);
        ...
        nativeScroller.prototype.onScroll = function () {           
            ...
            if (this.infinite && this.infiniteTriggered) {
                $.trigger(this, "infinite-scroll-end");
            }
        };
        ...
        nativeScroller.prototype.hideRefresh = function (animate) {
            ...
            var endAnimationCb = function (canceled) {
                //that.refreshContainer.style.top = "-60px"; //>>>deleted by wusheng
                that.refreshContainer.style[$.feat.cssPrefix + "Transform"] = "translate3d(0, -60px, 0)";
                ...
                if (!canceled) { //not sure if this should be the correct logic....
                    ...
                    that.setRefreshContent(that.pullRefreshTip, 'to_pull');
        ...
        jsScroller = function (el, opts) {
            ...
            if(opts.hasParent)
                ...
                $(this.el).append($div);
                this.container = this.el; //>>> fooWrap ***
                this.el = $div.get(0); //>>> div to panning
            }
            ...
            if (this.verticalScroll && this.verticalScroll === true && this.scrollBars === true) {
                scrollDiv = createScrollBar(2, 20);
                scrollDiv.style.right = "3px";
            ...
            if (this.horizontalScroll && this.horizontalScroll === true && this.scrollBars === true) {
                scrollDiv = createScrollBar(20, 2);
        ...
        jsScroller.prototype.addPullToRefresh = function (el, leaveRefresh) {
            ...
            if (this.refresh && this.refresh === true) {
                this.coreAddPullToRefresh(el);
                this.refreshContainer.style.display = "block"; //top:-60px;
        ...
        jsScroller.prototype.onTouchStart = function (event) {
            ...
            $(this.el).animateCss().stop(); //>>> stop the el from scrolling when you put finger on the scrolling div
            ...
            //element to be css moved(panned) info
            this.elementInfo = {};
            var size = this.getViewportSize();
            ...
            //get the current top
            var cssMatrix = this.getCSSMatrix(this.el);
            scrollInfo.top = numOnly(cssMatrix.f) - numOnly(this.container.scrollTop); //>>> css 3d panned cordinate
            scrollInfo.left = numOnly(cssMatrix.e) - numOnly(this.container.scrollLeft);

            this.container.scrollTop = this.container.scrollLeft = 0; //>>> so ...
            this.currentScrollingObject = this.el;

            //get refresh ready
            if (scrollInfo.top === 0 && this.refresh) {
                //this.refreshContainer.style.display = "block";
            ...
            } else if (!this.preventPullToRefresh && scrollInfo.top < 0) {
                this.preventPullToRefresh = true;
            ...
            //vertical scroll bar
            if (this.elementInfo.requiresVScrollBar && this.setVScrollBar(scrollInfo, 0, 0)) {
                this.vscrollBar.style[$.feat.cssPrefix + "Transition"] = "";
                $(this.vscrollBar).animateCss().stop();
            }

            //horizontal scroll
            if (this.elementInfo.requiresHScrollBar && this.setHScrollBar(scrollInfo, 0, 0)) {
            ...
            if(this.elementInfo.maxTop === 0 && this.elementInfo.maxLeft === 0 && this.lockBounce){
            	this.scrollToTop(0);
            	this.scrollerMoveCSS(this.lastScrollInfo, 0); //>>> wusheng
            }
            else
        ...
        jsScroller.prototype.setVScrollBar = function (scrollInfo, time) {
            var containerHeight = this.elementInfo.bottomMargin,
                newHeight = (parseFloat(containerHeight / this.elementInfo.divHeight) * containerHeight);
            if(newHeight > containerHeight) newHeight = containerHeight+"px";
            else newHeight = newHeight + 'px';
            if (newHeight !== this.vscrollBar.style.height)
                this.vscrollBar.style.height = newHeight;

            var leftHeight = containerHeight - numOnly(this.vscrollBar.style.height),
                pos = leftHeight - (((this.elementInfo.maxTop + scrollInfo.y) / this.elementInfo.maxTop) * leftHeight);

            this.scrollbarMoveCSS(this.vscrollBar, { x: 0, y: pos }, time);
            return true;
        };
        
        jsScroller.prototype.setHScrollBar = function (scrollInfo, time) { //>>> to be Optimized
            ...
            */
            ...
            if(false && $.os.ios) {
                ...
                if (positiveOverflow || negativeOverflow) {
                    ...
                    var absChangeY = Math.abs(changeY); //>>> wusheng
            ...
            //move
            this.scrollerMoveCSS(this.lastScrollInfo, 0);
            if (this.elementInfo.requiresVScrollBar) this.setVScrollBar(this.lastScrollInfo, 0, 0);
            if (this.elementInfo.requiresHScrollBar) this.setHScrollBar(this.lastScrollInfo, 0, 0); //>>> wusheng

            //check refresh triggering, >>> put to animateCss end
        ...
        jsScroller.prototype.hideRefresh = function (animate) {
            ...
            var endAnimationCb = function () {
                ...
                that.setRefreshContent(that.pullRefreshTip, 'to_pull');
                $.trigger(that, "refresh-finish");
                that.preventPullToRefresh = true;
                that.refreshContainer.style.overflow = "hidden";
            };
           
            if (animate === false) {
                endAnimationCb();
            } else {
                this.scrollerMoveCSS({x: 0, y: 0, clearDoing: true}, HIDE_REFRESH_TIME, endAnimationCb);
            }
            this.refreshTriggered = false;
        };
        ...
        jsScroller.prototype.setMomentum = function (scrollInfo) {
            ...
            if (scrollInfo.absDeltaY > 0) {
            	scrollInfo.absSpeedY = Math.abs(this.divide(scrollInfo.deltaY, scrollInfo.duration));
                ...
                if (scrollInfo.absSpeedY < deceleration * 300 || scrollInfo.absDeltaY < 20) //>>> wusheng
                    scrollInfo.deltaY = scrollInfo.absDeltaY = scrollInfo.duration = scrollInfo.speedY = scrollInfo.absSpeedY = 0;
                else scrollInfo.duration = scrollInfo.duration/1.5; //>>> 1.5 by wusheng
            } else if (scrollInfo.absDeltaX) {
            	scrollInfo.absSpeedX = Math.abs(this.divide(scrollInfo.deltaX, scrollInfo.duration));
                ...
                if (scrollInfo.absSpeedX < deceleration * 200 || scrollInfo.absDeltaX < 15) 
        ...
        jsScroller.prototype.onTouchEnd = function () {
            ...
            if (!this.androidFormsMode) this.setMomentum(scrollInfo);
            ...
            this.scrollerMoveCSS(scrollInfo, scrollInfo.duration);
            if (this.elementInfo.requiresVScrollBar) this.setVScrollBar(scrollInfo, scrollInfo.duration);
            if (this.elementInfo.requiresHScrollBar) this.setHScrollBar(scrollInfo, scrollInfo.duration);
        ...
        jsScroller.prototype.stopFormsMode = function () { //>>>not used
            if (this.blockFormsFix) return;
            //get prev values
            var cssMatrix = this.getCSSMatrix(this.el);
            //toggle vars
        ...
        jsScroller.prototype.scrollerMoveCSS = function (distanceToMove, time, completeCB) {
            time = time || 0;
            var self=this;
                ...
                if (this.eventsActive) {
                    if (this.androidFormsMode) {
                        ...
                        var opts={
                            ...
                            y:distanceToMove.y,
                            duration:time,
                            complete: completeCB || false,
                            clearDoing: distanceToMove.clearDoing || false,
                            easing:"easeOutSine"
                        };
        ...
        jsScroller.prototype.scrollbarMoveCSS = function (el, distanceToMove, time) {
            if (!time) time = 0;
    ...
    */
    ...
    $.selectBox = {
        ...
        getOldSelects: function(elID) {
            if (!$.os.android || $.os.androidVersion >= 4.0) return;
            if (!$.fn.scroller) {
...
(function($) {
    "use strict";
    var touch = {}, longTapTimer=null, pressed=false, dPressTimer = null, dPressDelay=180, longTapDelay = 750, touchMoveThresh = 3;
    ...
    $(document).ready(function() {
        ...
        $(document.body).bind("touchstart", function(e) {
            ...
            if (touch.el.attr("data-pressed")) {
                ...
                pressed = true;
            } else if(touch.el.attr("data-dpressed")) {
                dPressTimer = setTimeout(function(){touch.el.addClass("pressed");}, dPressDelay);
                pressed = true;
            }
                
            if (prevEl && (prevEl.attr("data-pressed") || prevEl.attr("data-dpressed")) && prevEl[0] !== touch.el[0])
            ...
            if(Math.abs(touch.x2-touch.x1) > touchMoveThresh || Math.abs(touch.y2-touch.y1) > touchMoveThresh) {
                if(longTapTimer){
                    clearTimeout(longTapTimer);
                    longTapTimer = null;
                }
            ...
            if(pressed) {
                if (dPressTimer){
                    clearTimeout(dPressTimer);
                    dPressTimer = null;
                    if((Date.now() - touch.last) < 200) { //single click auto pressed(it does not well on Android)
                        touch.el.addClass("pressed");
                        var tTouch = touch;
                        setTimeout(function(){tTouch.el.removeClass("pressed"); pressed = false;}, 200);
                    } else {
                        touch.el.removeClass("pressed");
                        pressed = false;
                    }
...
//It can be used independently in other apps but it is required by afUi
...
//keybord display mode:
//keyboard-show - keyboard show event
//keyboard-hide - keyboard hide enent
...
(function($) {
    ...
    var requiresJSFocus = $.os.ios || ($.os.androidVersion >= 4.0 && $.os.chrome); //devices which require .focus() on dynamic click events
    ...
    var inputElementRequiresNativeTap = $.os.blackberry || $.os.fennec || ($.os.android && $.os.androidVersion < 4.0); //devices which require the touchstart event to bleed through in order to actually fire the click on select elements
    ...
    var touchLayer = function(el) {
        this.clearTouchVars();
        el.addEventListener("touchstart", this, false); //>>> note the 2nd param is obj(cause the JS engine to look for handleEvent of this obj)
        ...
        this.exitEditProxy_ = $.proxy(this.exitEdit, this, []);
        ...
        $.bind(this, "scrollstart", function(el) { //>>> trigger by scroller plugin
        ...
        $.bind(this, "scrollend", function(el) {
            ...
            that.fireEvent("UIEvents", "scrollend", el, false, false); //>>> it is async, you shoul bind handler to this el, not the scroller
        });
        
        //fix layer positioning
        if(!$.ui || !$.ui.isIntel) this.hideAddressBar(0,1);
    ...
    touchLayer.prototype = {
        ...
        handleEvent: function(e) {
            switch (e.type) {
                ...
                case "blur":
                    this.onBlur(e);
                    break;
                case "focusout": //>>> ios fire this event, not android
                    this.onFocusOut(e);
                    break;
                case "scroll":
                    this.onScroll(e);
                ...
                case "orientationchange":
                    ...
                    break;
                case "resize":
                    this.onResize(e);//>>> android fire this event, not ios
                    break;
                case "focusin":
                    this.onFocusIn(e);
                    break;
        ...
        testAndFixUI: function(retry, maxTries) {
            ...
            if ((refH !== curH && !(curH * addressBarError < refH && refH * addressBarError < curH))) {
                ...
                if(!$.ui || !$.ui.isIntel) this.hideAddressBar(retry, maxTries);
                return true;
            }
            if ($.os.android) this.resetFixUI();
            return false;
        },
        //hide browser address bar, not useful for cordova
        hideAddressBar: function(retry, maxTries) {
            ...
            //console.log("hiding address bar");
            if($.os.ios){
                window.scrollTo(1, 1);
            }
        ...
        onResize: function(e) { //>>> ios not trigger this event(alougth the window size change)
            //avoid infinite loop on iPhone
            if (this.ignoreNextResize_) {
                //console.log("ignored resize");
                this.ignoreNextResize_ = false;
                return;
            }
            //console.log("resize");
            if (this.launchFixUI()) {
                if(!$.os.android) this.reshapeAction();
                else {
                    var curWindowHeight = window.innerHeight, that = this; //>>> for ios: $.ui.fullWindowHeight always equals to curWindowHeight
                    if($.ui.fullWindowHeight > curWindowHeight && this.clickedInputEle && !this.keyboardShowed) {
                        this.previewReshapeEvent("keyboard-show", 50);
                        this.keyboardShowed = true;
                    } else if($.ui.fullWindowHeight == curWindowHeight && this.keyboardShowed) {
                        this.previewReshapeEvent("keyboard-hide", ($.os.androidVersion > 4.0 ? 50 : 750));
                        window.setTimeout(function(){that.keyboardShowed = false;}, 200); //>>> for backButton
                    }
                }
            }
        },
        onClick: function(e) {
            this.clickedInputEle = e.target;
            //handle forms
            var tag = e.target && e.target.tagName !== undefined ? e.target.tagName.toLowerCase() : "",
                isInputEle = inputElements.indexOf(tag) !== -1;
            if(isInputEle) this.clickedInputEle = e.target;
            //console.log("====== touchlayer click on =======> "+tag);
            if (isInputEle && (!this.isFocused_ || e.target !== (this.focusedElement))) {
        ...
        previewReshapeEvent: function(ev, time) {
            time = time || 750;
        ...
        fireReshapeEvent: function(ev) {
            //console.log(ev?ev+"-reshape":"unknown-reshape");
        ...
        reshapeAction: function(ev) {
            ...
            } else this.previewReshapeEvent(ev);
        },
        ...
        onScroll: function(e) {
            //console.log("document scroll detected "+document.body.scrollTop);
            if (!this.allowDocumentScroll_ && !this.isPanning_ && e.target === document) {
                ...
                if (this.wasPanning_) {
                    ...
                    //console.log("scroll forced page into place");
                    this.launchFixUI();
                }
            }
        },
        ...
        onTouchStart: function(e) {
            ...
            if ($.feat.nativeTouchScroll) this.checkDOMTree(e.target, this.layer); //ppp
            //scrollend check
            ...
            if($.os.chrome || $.os.fennec || $.os.androidVersion >= 4.0) return;
        ...
        demandVerticalScroll: function() {
            ...
            if (atTop) {
                //console.log("adjusting scrollTop to 1");
                ...
                if (atBottom) {
                    //console.log("adjusting scrollTop to max-1");
        ...
        onTouchEnd: function(e) {
            ...
            if (verySensitiveTouch) {
                itMoved = itMoved && (Math.abs(this.cY) > 10 || Math.abs(this.cX) > 10);
            }
            ...
            if (this.isPanning_ && itMoved) {
                ...
                if (this.isScrolling) {
                    ...
                    if (Math.abs(this.speedY) < 0.01) {
                        ...
                        //console.log(" scrollend immediately "+this.speedY);
                        this.scrollEnded(false);
                    } else {
                        //wait for scroll event
                        //console.log($.debug.since()+" setting scroll timeout "+this.speedY);
        ...
        clearTouchVars: function() {
            //console.log("clearing touchVars");
...
/* EXAMPLE
 $.query("body").popup({
        ...
        supressTitle:false //Do not show the title if set to true
  });
...
(function ($) {
    "use strict";
    var afuiPopup = null; //single instance
    
    $.fn.popup = function (opts) {
    	if(!opts) return afuiPopup;
    	else if(!afuiPopup) afuiPopup = new popup(this[0], opts);
    	else if(afuiPopup.id != opts.id){
    		afuiPopup.hide();
    		afuiPopup = new popup(this[0], opts);
    	}
        return afuiPopup;
    };
    ...
    var popup = (function () {
        var popup = function (containerEl, opts) {
            ...
            try {
                ...
                this.supressTitle = opts.supressTitle || this.supressTitle;
                this.title = opts.supressTitle ? "" : (opts.title || "提示");
                this.message = opts.message || '<p class="af_loading"><i class="fa fa-spinner fa-spin pink"></i>加载中...</p>';
                this.cancelText = opts.cancelText || "取消";
                this.cancelCallback = opts.cancelCallback || function () {};
                this.cancelClass = opts.cancelClass || "gbtn btn";
                this.doneText = opts.doneText || "确定";
                this.doneCallback = opts.doneCallback || function () {};
                this.doneClass = opts.doneClass || "gbtn btn";
                this.cancelOnly = opts.cancelOnly || false;
                this.blockUI = opts.blockUI || false;
                this.supressFooter = opts.supressFooter === false ? false : true;
        ...
        popup.prototype = {
            ...
            show: function () {
                ...
                if(!this.supressFooter){
                	footer = '<footer>'+
		                    	'<a data-pressed="true" class="' + this.cancelClass + ' optbtn" id="cancel">' + this.cancelText + '</a>'+
		                    	'<a data-pressed="true" class="' + this.doneClass + ' optbtn" id="action">' + this.doneText + '</a>'+
		                    ' </footer>';
                }
                if(!this.supressTitle){
                	header = '<header><span>' + this.title + '</span><a id="pclose" class="fr optbtn" data-pressed="true">╳</a></header>';
                }
                var markup = '<div id="' + this.id + '" class="afPopup hidden '+ this.addCssClass + '">'+
                				header+'<div>' + this.message + '</div>'+footer+'</div>';
                ...
                if (this.cancelOnly && !this.supressFooter) {
                    $el.find('a#action').hide();
                    $el.find('a#cancel').addClass('center');
                }

            	$el.find('.optbtn').each(function () {
                    var button = $(this), bid = button.attr('id');
                    button.bind('click', function (e) {
                        if (bid == 'cancel' || bid == 'pclose') {
                ...
                setTimeout(function(){
                    $el.addClass("showed");
                    self.onShow(self);
                    if($.os.android) { //fix android hairline
                        setTimeout(function(){
                            if($('.afPopup > FOOTER .button').length > 1){
                                var preBtn = $('.afPopup > FOOTER .button:first-child'),
                                    nxtBtn = $('.afPopup > FOOTER .button:last-child'), 
                                    tPW = preBtn.width(), tNW = nxtBtn.width(),
                                    tPIW = parseInt(tPW);
                                if(tPIW % 2 == 0) {
                                    preBtn.width((tPW+1)+'px'),
                                    nxtBtn.width((tNW-1)+'px');
                                }
                            }   
                        }, 200);
                    }
            ...
            hide: function () {
                var self = this;
                $.query('#' + self.id).removeClass('showed');
                if(this.blockUI) $.unblockUI();
                if(!$.os.ie && !$.os.android){
                    setTimeout(function () {
                        self.remove();
                    }, 250);
            ...
            setCnt: function (html) { $.query('#' + this.id + '>div').html(html); },
            setTitle: function (html) { $.query('#' + this.id + '> header > span').html(html); },
            remove: function () {
            	afuiPopup = null;
                var self = this;
            ...
            positionPopup: function () {
            	if(this.addCssClass) return false;
    ...
    $.blockUI = function (opacity) {
        ...
        $.query("#mask").bind("touchstart", function (e) {
            e.stopPropagation(); //>>> prevent Android < 4.4 clicking through
            //e.preventDefault(); //>>> we need click event
        });
        $.query("#mask").bind("touchmove", function (e) {
            e.stopPropagation(); //>>> we just intercept all the events to us
            e.preventDefault();
        });
        uiBlocked = true;
    };
    ...
    $.unblockUI = function () {
        uiBlocked = false;
        $.query("#mask").unbind("touchstart touchmove").remove();
    };
})(af);
...
(function($) {
    ...
    var ui = function() {
        ...
        var setupAFDom = function() {
            ...
            that.isIntel = $(afui).attr('data-intel') == 'y';
        ...
        function setupCustomTheme() {
            if (that.useOSThemes) {
                $("#afui").removeClass("ios ios8 win8 tizen bb android light dark firefox");
            ...
            else if ($.os.android && $.os.androidVersion < 4.0){
                that.transitionTime = "200ms";
            }
            else if($.os.fennec){
                that.ready(function(){
    ...
    ui.prototype = {
        ...
        ajaxUrl: "",
        transitionType: "none", //>>> changed by wusheng
        scrollingDivs: {},
        firstDiv: "",
        hasLaunched: false,
        isLaunching:false,
            ...
            if (this.autoLaunch) {
                if(this.isIntel){
                    var xdkLaunch=function(){
                        that.launch();
                        document.removeEventListener("deviceready",xdkLaunch);
                    };
                    document.addEventListener("deviceready",xdkLaunch);
                }
                else {
                    this.launch();
                }
        ...
        enableTabBar:function(){
            $(document).on("click",".button-grouped.tabbed",function(e){
                var $el=$(e.target);
                $el.closest(".tabbed").find(".button").removeAttr("data-pressed").removeClass("pressed");
        ...
        disableTabBar:function(){
            ...
            $(".button-grouped.tabbed .button").attr("data-pressed", 'auto');
        },
        /**
        ...
        pushHistory: function(previousPage, newPage, transition, hashExtras) {
            ...
            this.history.push({
                target: previousPage, //previousPage = hashID+hashExtras
                transition: transition,
                newTarget: "#" + newPage + hashExtras
            });
        ...
        */
        ...
        updateNavbarElements: function(elems) {
            if (this.prevFooter) {
                ...
                else this.prevFooter.appendTo("#afui"); //>>> step1: save to other place
            }
            if (!$.is$(elems)) //inline footer
            {
                elems = $.query("#" + elems);
            }
            $.query("#navbar").append(elems); //>>> step2: update the footer nav
            this.prevFooter = elems;
            var tmpAnchors = $.query("#navbar > footer > a:not(.button)");
            if (tmpAnchors.length > 0) {
                tmpAnchors.removeAttr("data-pressed"); //>>> wusheng del: .data("resetHistory", "true")
        ...
        updateHeaderElements: function(elems, goBack) {
            ...
            //fix two sequently goBack bug...
            var animator = this.prevHeader.css3Animate();
            if(animator && animator.isActive){
                animator.finishAnimation(); //this is a Synchronous operation
            }
            ...
            if (this.prevHeader) {
                ...
                //console.log('>>>>>>>>>transitionType:: '+that.transitionType);
                that.css3animate(that.prevHeader, {
                    x: from,
                    y: 0,
                    opacity: 0.3,
                    time: that.transitionType == 'none' ? '0ms' : that.transitionTime,
                    delay: that.transitionType == 'none' ? '5ms' : numOnly(that.transitionTime) / 5 + "ms",
                    complete: function() {
                        if (that.prevHeader.data("parent")){
                            if($.feat.nativeTouchScroll||$.os.desktop || !useScroller ){
        ...
        updateLeftSideMenuElements:function(elems) {
            ...
            if (this.prevMenu) {
                this.prevMenu.insertBefore("#afui #menu"); //>>> step1: save the prev nav
                this.prevMenu = null;
            }
            ...
            nb.html("");
            nb.append(elems); //>>> step2: set the new nav
            this.prevMenu = elems;
            //Move the scroller to the top and hide it
        ...
        showModal: function(id, trans) {
            ...
            if ($panel.length) { //>>> panel must be loaded first
        ...
        addDivAndScroll: function(tmp, refreshPull, refreshFunc, container) {
            ...
            if($(tmp).hasClass("no-scroll") || (tmp.getAttribute("scrolling") && tmp.getAttribute("scrolling") === "no")) { 
                hasScroll = false;
                jsScroll = false;
                tmp.removeAttribute("js-scrolling"); //>>> override js-scrolling attr
            }
            
            container.appendChild(tmp); //>>> add tiv pannel
            scrollEl = tmp;
         ...
         * @api private
         */
        parsePanelFunctions: function(what, oldDiv, goBack, transition) {
            ...
            if (oldDiv) {
                var fnc = oldDiv.getAttribute("data-unload");
                if(fnc)
                    this.dispatchPanelEvent(fnc, oldDiv);
                window.setTimeout(function(){
                        $(oldDiv).trigger("unloadpanel", {'goBack': goBack});
                    }, 
                    ((transition != 'none' ? numOnly(that.transitionTime) : 0) + 100) 
                ); //>>> wusheng: loadpanel moved to loadContentData
            }
         ...
         * We can pass in a hash+id or URL and then we parse the panel for additional functions
           ...
           ```
         * @param {string} target
         * @param {boolean=} resetHistory (resets history)
         * @param {boolean=} go back (initiate the back click)
         * @param {string=} transition
         * @param {object=} anchor
         * @title $.ui.loadContent(target, resetHistory, goBack, transition, anchor);
         * @api public
         */
        loadContent: function(target, resetHistory, back, transition, anchor) {
            if (this.doingTransition) {
                this.loadContentQueue.push([target, resetHistory, back, transition, anchor]);
                return;
            }
            ...
            if (target.indexOf("#") === -1 && loadAjax) {
                this.loadAjax(target, resetHistory, back, transition, anchor);
            } else {
                this.loadDiv(target, resetHistory, back, transition);
            }
        },
        /**
         ...
         * @param {string=} transition
         * @title $.ui.loadDiv(target,resetHistory,goBack,transition);
         * @api private
         */
        loadDiv: function(target, resetHistory, back, transition) {
            //load a div
            var that = this;
            ...
            if (slashIndex !== -1) {
                ...
                hashExtras = what.substr(slashIndex).trim();
                what = what.substr(0, slashIndex);
            }

            //lazyload panel
        	var hashId = what, 
        	    pn = this.panels[hashId], 
                what = $.query("#" + hashId).get(0);
        	...
        	if(!what) {
                ...
                $(this.content).append('<div id="'+hashId+'" title="'+pn.title+'" class="panel" ' + (pn.scroll ? '' : 'style="overflow:hidden"') + ' data-footer="'+pn.footer+'" ' + (pn.scrollBar ? 'data-scrbar="y"': '') + ' data-tab="'+pn.tab+'"></div>');
                what = $.query("#" + hashId).get(0);
                this.addDivAndScroll(what);
                
                if(pn.view){
                	 $.ajax({
                         url:intel.xdk.webRoot + 'views/'+pn.view+'.html?rnd='+(new Date()).getTime(),
                         async:true,
                         success: function(data) {
                        	 that.updatePanel(hashId, data);
                        	 window.setTimeout(function(){
                                    $.loadJS('./js/'+pn.js+'.js', function(){
                                    that.loadDiv(target, resetHistory, back, transition);
                                 });
                        	 }, 1); //1ms gived to browser to render the html
                         },
                         error: function(xhr, why) {
                             //load error, just clean the panel and do nothing
                             var errPanel = $.query("#" + hashId);
                             $.cleanUpContent(errPanel.get(0), true, true);
                             errPanel.remove();
                        	 console.log('=============== load panel error:: '+why+' ================');
                         }
                     });
                }
                //console.log('-------------------------> load panel:: '+hashId+' ================');
                return;
        	}
        	
            //>>> page params
            var tparams = hashExtras.length > 1 ? hashExtras.substr(1).split('/') : [];
            tparams = $.isArray(tparams) ? tparams : [tparams] ; //split bug
            
            $(what).data('params', tparams);

            //stop the old ajax Request
            $.abortQueuedAjax();

            if (what === this.activeDiv && !back) {
                //toggle the menu if applicable
                if (this.isSideMenuOn()) this.toggleSideMenu(false); 
                if (this.transFinishCb) this.transFinishCb();
                $(what).trigger('loadpanel', {'goBack': false}); //>>> //go to self but with different params
                return;
            }
            
            transition = this.transitionType = transition ? transition : $.ui.transition;
            var oldDiv = this.activeDiv;
            var currWhat = what;
            
            if (oldDiv === currWhat) {//prevent it from going to itself
            	$(what).trigger('loadpanel', {'goBack': back}); //go to self but with different params
                return;
            }
            
            if(this.onLoadDivHook){
                if(this.onLoadDivHook(hashId, tparams, back) !== true) return;
            }
            ...
            if (resetHistory) {
                //support second level resetHistory
                if(this.history.length > 0) {
                    var tmpHist = this.history[0], tmpTransition = tmpHist.transition, tmpTarget = tmpHist.newTarget.replace('#', ''),
                        tmpIndex = tmpTarget.indexOf("/"), tmpTargetId = tmpTarget, tmpHashExtras = '';
                    if(tmpIndex != -1) {
                        tmpHashExtras   = tmpTarget.substr(tmpIndex);
                        tmpTargetId     = tmpTarget.substr(0, tmpIndex);
                    } 
                    this.clearHistory();
                    this.pushHistory("#" + this.firstDiv.id, tmpTargetId, tmpTransition, tmpHashExtras);
                    this.pushHistory(tmpHist.newTarget, what.id, transition, hashExtras);  
                } else {
                    this.clearHistory();
                    this.pushHistory("#" + this.firstDiv.id, what.id, transition, hashExtras); 
                }
            } else if (!back) {
                this.pushHistory(previousTarget, what.id, transition, hashExtras);
            }

            previousTarget = "#" + what.id + hashExtras;
            ...
            */
            ...
            this.parsePanelFunctions(what, oldDiv, back, transition);
            //Need to call after parsePanelFunctions, since new headers can override
            this.loadContentData(what, resetHistory, back, transition);
         ...
         * @param {boolean=} resetHistory (resets history)
         * @param {boolean=} go back (initiate the back click)
         * @title $.ui.loadContentData(target,resetHistory,goBack);
         * @api private
         */
        loadContentData: function(what, resetHistory, back, transition) {
            var prevId, el, val, slashIndex, that=this;
            if (back) {
                if (this.history.length > 0) {
                    ...
                    else this.setBackButtonText("后退");
                }
            ...
            //>>> wusheng: moved from parsePanelFunctions
            var fnc = what.getAttribute("data-load");
            if(fnc)
            	this.dispatchPanelEvent(fnc,what,{'goBack': back});
            	
            setTimeout(function(){
                    $(what).trigger("loadpanel", {'goBack': back});
                }, 
                transition != 'none' ? ($.os.ios ? 10 : numOnly(that.transitionTime)) : 10
            );
         ...
         * @api private
         */
        loadAjax: function(target, resetHistory, back, transition, anchor) {
            // XML Request
                ...
                success:function(responseText){
                    ...
                    if (retainDiv.length > 0) {
                        ...
                        refreshFunction = refresh ? function() {
                            setTimeout(function() {
                                anchor.refresh = true;
                                that.loadContent(target, resetHistory, back, transition, anchor);
                        ...
                        that.loadContent("#afui_ajax", resetHistory, back, transition);
                    ...
                    that.loadContent("#" + urlHash, resetHistory, back, transition);
                    if (that.showLoading) that.hideMask();
                    return null;
                },
                error:function(){
        ...
        runTransition: function(transition, oldDiv, currWhat, back) {
            if (!this.availableTransitions[transition]) transition = $.ui.transition; //changed by wusheng
            if(transition != 'none'){
                if(oldDiv.style.display!="block") oldDiv.style.display = "block";
                if(currWhat.style.display!="block")currWhat.style.display = "block";
            }
        ...
        launch: function() {
            ...
            if(this.isLaunching) return true;
            this.isLaunching=true;
            
            var that = this;
            setTimeout(function(){that.fullWindowHeight = window.innerHeight;}, 1000); //>>> android < 4.2 can't get the right height sometimes
            ...
            //enter-edit-reshape panel padding and scroll adjust:: for ios: TODO
            if(false && $.os.ios) {
                $.bind($.touchLayer, "enter-edit-reshape", function() {
                    //onReshape UI fixes
                    //check if focused element is within active panel
                    var afEditEl = $(enterEditEl);
                    //console.log('+++++++++++ enter-edit-element:: ' + enterEditEl.tagName);
                    var activePanel = afEditEl.closest(that.activeDiv);
                    if (activePanel && activePanel.size() > 0) {
                        if($.os.android && $.os.androidVersion < 4.4 && $.os.androidVersion >= 4.0){ ///
                            //console.log('+++++++++++ iosVersion < 4.4::', $.os.androidVersion);
                            //get the top
                            ...
                            setTimeout(
                                function(){
                                    scroller.scrollBy({
                                        x:0,
                                        y:((afEditEl.offset().top - activePanel.offset().top+top)/2)
                                    });
                                },
                                50
                            );
                            ...
                            //console.log('+++++++++++ iosVersion > 4.4::', $.os.androidVersion);
                            var elPos = afEditEl.offset();
                            var containerPos = activePanel.offset();
                            if (elPos.bottom > containerPos.bottom && elPos.height < containerPos.height) {
                                /*apply fix >>> align the input element bottom with the bottom of the scroller container: 
                                 50ms let the browser render OK
                                 */
                                setTimeout(function(){that.scrollingDivs[that.activeDiv.id].scrollToItem(afEditEl, 'bottom');}, 50);
                            }
                        }
                    }
                });
             ...
             //enter-edit-reshape panel padding and scroll adjust: for andorid
            if($.os.android) {
                $.bind($.touchLayer, "keyboard-show-reshape", function() { //>>>***
                    //onReshape UI fixes
                    //check if focused element is within active panel
                    var afEditEl = $(this.clickedInputEle);
                    var scroller=that.scrollingDivs[that.activeDiv.id];
                    if(!scroller) return;
                    var scrollTop = scroller.scrollTop,
                        activePanel = afEditEl.closest(that.activeDiv);
                    if (activePanel && activePanel.size() > 0) {
                        var elPos = afEditEl.offset();
                        var containerPos = activePanel.offset();
                        if ((elPos.bottom > containerPos.bottom) || (elPos.top < containerPos.top && scrollTop < 0)) {
                            /*apply fix >>> align the input element bottom with the bottom of the scroller container: 
                             50ms let the browser render OK
                             */
                            if(elPos.height < containerPos.height) {
                                setTimeout(function(){that.scrollingDivs[that.activeDiv.id].scrollToItem(afEditEl, 'bottom');}, 50);
                            }
                        }
                    }
                    //this.clickedInputEle = null;
                });
                
                $.bind($.touchLayer, "keyboard-hide-reshape", function() {
                    if (that.activeDiv && that.activeDiv.id && that.scrollingDivs.hasOwnProperty(that.activeDiv.id)) {
                        $(that.activeDiv).find('input, textarea, range').each(function(){
                            this.blur();
                        });
                        $(that.activeDiv).click();
                        var scroller=that.scrollingDivs[that.activeDiv.id];
                        if(!scroller) return;
                        scroller.correctScroll(100);
                    }
                });
            }
            ...
            $(this.header).html("<a class='backButton fa fa-angle-right fa-flip-horizontal' data-pressed='true'></a> <h1 class='pageTitle'></h1>" + this.header.innerHTML);
            ...
            var maskDiv = $.create("div", {
                id: "afui_mask",
                className: "ui-loader",
                html: "<span class='ui-icon ui-icon-loading spin'></span><h1>加载中...</h1>"
            }).css({
                "z-index": 20000,
                display: "none"
            });
            ...
            var defer = {}; 
            //>>> wusheng
            var defPanel = localStorage.getItem('default_panel');
            defPanel = defPanel ? defPanel : $.ui.defaultPanel;
            var pm = $.ui.panels[defPanel];
            $(this.content).append('<div selected="true" title="'+pm.title+'" id="'+defPanel+'" class="panel" data-defer="views/'+pm.view+'.html" '+(pm.scroll ? '' : 'style="overflow:hidden"')+' '+(pm.scrollBar ? 'data-scrbar="y"': '')+' data-footer="'+pm.footer+'" data-tab="'+pm.tab+'"></div>');
            ...
            for (var i = 0; i < contentDivs.length; i++) {
                ...
                if (el.parentNode && el.parentNode.id !== "content") {
                    ...
                    //$(el).insertAfter(prevSibling); //>>> wusheng
                }
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.length > 0) {
                            ...
                            //that.parseScriptTags($.query("#" + j).get(0)); //>>> wusheng: ajax will parseJS the data
                        }
                        loaded++;
                        if (loaded >= toLoad) {
                            loadingDefer = false;
                ...
                var loadFirstDiv = function() {
                    ...
                    */
                    ...
                    if (firstPanelId.length > 0 && that.loadDefaultHash && !isFirstPanel) {
                        that.loadContent(defaultHash, true, false, "none"); //load the active page with no transition and reset history
                    } else {
                        previousTarget = "#" + that.firstDiv.id;
                        that.firstDiv.style.display = "block";
                        //>>> wusheng: fail if the firstDiv has no JS
                        if(that.firstDiv.id != 'afui_ajax'){
                            window.setTimeout(function(){
                                $.loadJS('./js/'+that.firstDiv.id+'.js', function(){
                                     //Let's check if it has a function to run to update the data
                                     ...
                                     if (that.firstDiv.getAttribute("data-modal") === "true" || that.firstDiv.getAttribute("modal") === "true") {
                                         that.showModal(that.firstDiv.id); //>>> show it in modal mode
                                     }
                                }); 
                            }, 1);
                        }
        ...
        topClickScroll: function() {
            ...
            document.getElementById("header").addEventListener("click", function(e) {
               // if (e.clientY <= 15 && e.target.nodeName.toLowerCase() === "h1") //hack - the title spans the whole width of the header
            	if (e.target.nodeName.toLowerCase() === "h1" && that.activeDiv.getAttribute('scrollTgt')) //>>> by wusheng...
                    $.query(that.activeDiv.getAttribute('scrollTgt')).scroller().scrollToTop(300);
            });
        ...
        noTransition: function(oldDiv, currDiv) {
            ...
            */
            ...
            this.finishTransition(oldDiv); //>>> wusheng
        },
        /**
    ...
    var checkAnchorClick = function(e, theTarget) {
        ...
        */
        ...
        if (theTarget.tagName.toLowerCase() !== "a" || theTarget.getAttribute("data-ignore")) return;
        
        //anchors
        var custom = (typeof $.ui.customClickHandler === "function") ? $.ui.customClickHandler : false,
            href = theTarget.href;
        if(!href || theTarget.getAttribute('href').length < 1) return e.preventDefault();
        if (custom !== false) if($.ui.customClickHandler(theTarget,e)) return e.preventDefault();
        ...
        if (href.indexOf('http') == 0) {
            if (href.toLowerCase().indexOf("javascript:") !== 0) {
                if ($.ui.isIntel) {
                    ...
                    $.ui.launchExternal(href); //TODO: open IN external browser/inAppBrowser
                } else if (!$.os.desktop) {
        ...
        if(href.indexOf(":") !== -1 && urlRegex.test(href)) e.preventDefault();
...
(function($) {
    ...
    var xdkDeviceReady=function(){
        ...
        document.removeEventListener("deviceready",xdkDeviceReady);
    };
    document.addEventListener("deviceready",xdkDeviceReady);
    ...
    function wire8Tiles() {
        ...
        $.ui.ready(function() {
            ...
            if (tmpAnchors.length > 0) {
            	...
            	tmpAnchors.removeAttr("data-pressed");//>>> wusheng
                var width = parseFloat(100 / tmpAnchors.length);
                tmpAnchors.css("width", width + "%");
            }
